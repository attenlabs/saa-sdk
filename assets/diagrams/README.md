...
- Card fills: neutral `#ffffff` / stroke `#e5e7eb`. Positive (addressed): `#F7F6F2` / stroke `#E15D35 sw 1.5`. Dark (cloud): `linear(#151513 → #2B2B28)`. All `rx: 12`.
- Background gradient `#bg` = `linear top→bottom · #F7F6F2 → #F7F6F2`.
- Accent (decision-positive only): primary `#E15D35` · fill `#F7F6F2`.
- Grays (side speech, dropped): bg `#f3f4f6` · stroke `#d1d5db / #9ca3af` · text `#6b7280` · dashed `4 4`.
- Arrows: neutral `stroke #A8A39A sw 1.5` · addressed `stroke #E15D35 sw 2`.
- Adapter brand accents live **only** on the adapter's own pill. The SAA gate keeps the canonical accent (terracotta `#E15D35`).
...
3. Decision-positive elements use the canonical accent (terracotta `#E15D35`); everything else stays neutral.
