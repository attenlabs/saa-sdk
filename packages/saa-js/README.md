...
JavaScript SDK for [Attention Labs](https://attentionlabs.ai) real-time selective auditory attention.
...
| ------------------ | -------- | ------------------------------------ | ----------- |
| `token`            | string   | _required_                           | Your API token from the dashboard. |
...
| `state`          | `{ state }` where state is one of `listening`, `sending`, `cancelled`, `idle` |
