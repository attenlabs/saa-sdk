...
Get your API token at [attentionlabs.ai/dashboard](https://attentionlabs.ai/dashboard). The token is required for every connection.
...
Register handlers with decorators. All callbacks fire on internal threads, so keep them fast or hand work off to your own thread.
...
| `@on_started`         | (none)                                                                   | Server-side warmup complete             |
| `@on_warmup_complete` | (none)                                                                   | First non-zero-confidence prediction    |
...
LLM routing is intentionally **not** part of the SDK. The `speech_ready` event hands you PCM16 audio (both as a NumPy array and as base64), so forward it wherever you like.
