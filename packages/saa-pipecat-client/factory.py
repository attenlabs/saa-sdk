...
def build_attention_runner(
    *,
    ...
    function that:
      ...
      4. Awaits the engine's `started` handshake, then returns
         `(engine, session)` to the caller.

    The returned coroutine completes when the engine is ready
    the caller runs their `PipelineRunner` and in a `finally:` block,calls
    ...
    async def run(
        ...
        task: Any,
    ) -> tuple[AttentionEngine, SessionHandle]:
        missing = [
        ...
        # Teardown is the caller's `finally:` block: engine.stop() clears
        # local state, session.stop() disconnects the hosted bot. Both are
        # returned so the caller can reach them.
        await engine.start()
