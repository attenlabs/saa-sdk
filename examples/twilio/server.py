...
* gates every call through SAA using ``upstream_mode=True`` plus the SDK's
  public ``feed_audio()`` API (no local mic / cam are ever opened)
...
DEFAULT_TOKEN = os.environ.get("ATTENLABS_TOKEN", "")
...
class _TwilioCallSession:
    ...
    async def mark_responding(self, responding: bool) -> None:
        ...
        trigger a second ``speech_ready``. The adapter calls this
    ...
    async def set_threshold(self, value: float) -> None:
        ...
        device-directed (fewer false speech_ready firings, more missed
...
async def ready() -> Response:
    ...
    if not DEFAULT_TOKEN:
        return JSONResponse(
            status_code=503,
            content={"status": "ATTENLABS_TOKEN unset"},
        )
...
async def twilio_media_stream(ws: WebSocket) -> None:
    ...
    if not DEFAULT_TOKEN:
        log.error("ATTENLABS_TOKEN not set, closing call.")
    ...
    # AttentionClient is constructed with upstream_mode=True so the SDK
    # never opens the host's mic/camera; phone audio reaches the cloud
    # exclusively via feed_audio() below. enable_audio=False /
    # enable_video=False are redundant under upstream_mode but kept
    # explicit so a reader scanning this file sees the intent.
    saa = AttentionClient(
        token=DEFAULT_TOKEN,
        url=DEFAULT_SAA_URL,
        upstream_mode=True,
        enable_audio=False,    # we inject Twilio audio ourselves
    ...
    @saa.on_speech_ready
    def _on_speech_ready(event):
        stats.speech_ready_count += 1
    ...
    @saa.on_reconnecting
    def _on_saa_reconnecting(event):
        stats.saa_reconnects += 1
        log.info(
            "[saa %s] reconnect attempt %d in %.1fs (cause=%s)",
            conn_id, event.attempt, event.delay_ms / 1000.0, event.cause_reason or "-",
        )
    ...
    async def _paced_outbound_sender() -> None:
        ...
        ``speech_ready`` on the echo (feedback loop). The bridge can also
    ...
    try:
        while True:
            ...
            if event == "connected":
                ...
                saa.start()
                saa_started = True
                # Block briefly for SAA's `started` (warmup-complete) signal
                # so the first 100 ms of caller audio actually hits a warm
                # model instead of getting dropped into the cloud's warmup
                # window. wait_ready blocks on a threading.Event so it must
                # go through asyncio.to_thread to avoid stalling the loop.
                # Falling through on timeout is intentional: a slow cloud
                # is preferable to dropping the call.
                ready_ok = await asyncio.to_thread(
                    saa.wait_ready, SAA_WAIT_READY_SECONDS,
                )
                if not ready_ok:
                    log.warning(
        ...
        with _AGGREGATE_LOCK:
            ...
            _AGGREGATE_STATS["speech_ready_total"] += stats.speech_ready_count
        ...
        log.info(
            ...
            "in=%d out=%d speech=%d barge_ins=%d saa_errors=%d saa_reconnects=%d",
            conn_id, stream_sid, duration,
            stats.audio_bytes_in, stats.audio_bytes_out, stats.speech_ready_count,
