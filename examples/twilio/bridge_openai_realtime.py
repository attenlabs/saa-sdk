...
* **Pre-ASR gating.** Only ``speech_ready`` payloads are ever sent to the
...
# Realtime input audio sample rate. Both Realtime and SAA hand us
# PCM16 @ 16 kHz, so the bridge ships SAA's speech_ready PCM straight
# through with no resampling.
REALTIME_INPUT_SAMPLE_RATE = 16_000
REALTIME_OUTPUT_SAMPLE_RATE = 16_000
...
class OpenAIRealtimeBridge:
    ...
    model:
        Realtime model name. Defaults to ``gpt-4o-realtime-preview``.
    voice:
    ...
    async def open(self, ctx: CallContext, session: CallSession) -> None:
        ...
        # Realtime requires the OpenAI-Beta header so the wire schema is
    ...
    async def on_speech(self, audio_pcm16_16k: np.ndarray, duration_sec: float) -> None:
        ...
        audio_b64 = base64.b64encode(
            np.ascontiguousarray(audio_pcm16_16k, dtype=np.int16).tobytes()
        ).decode("ascii")
        await self._send_json({
