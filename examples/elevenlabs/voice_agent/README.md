...
- `SAAFeedAudioInterface(DefaultAudioInterface(), saa)` -> wraps ElevenLabs' audio interface. Its mic tee feeds **every** frame to SAA, but sends ElevenLabs the user's **real audio only** while SAA says device-directed (and the agent isn't speaking, so its own echo never loops back), and **nothing** between turns. When SAA reports the turn is done (`on_turn_ready`), a short **silence tail** is streamed until the agent replies so ElevenLabs' turn model endpoints — there's no continuous silence stream. ElevenLabs keeps doing its own VAD/endpointing on that; SAA just decides what it hears.
- `@saa.on_prediction` → `attn.update_gate(ev.cls)` -> **the gate**: it opens on class-2 (device-directed), closes immediately on class-1 (human-directed). ElevenLabs endpoints on the silence tail streamed after `on_turn_ready`, then replies.
